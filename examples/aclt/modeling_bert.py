...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)  # 1.2
        self.dropout = nn.Dropout(config.hidden_dropout_prob)  # 1.3
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states, attention_mask, head_mask[
                    i], encoder_hidden_states, encoder_attention_mask
            )
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states,root_ids):
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.str_att = StructuredAtt(config)
    ...
    Examples::
        ...
        )  #  embeddings vecs -> self-attention*N
        sequence_output = encoder_outputs[0]  # B * token * Hidden size

        structured_output, attention_matrix, loss_root = self.str_att(
            sequence_output, extended_attention_mask, root_ids, root_mask)
        pooled_output = self.pooler(structured_output,root_ids)  # B* Hidden


        outputs = (sequence_output, pooled_output,) + encoder_outputs[
            1:]  # add hidden_states and attentions if they are here

        return outputs, loss_root
...
class BertForMaskedLM(BertPreTrainedModel):  # fixme
    ...
    Returns:
        ...
        if masked_lm_labels is not None:
            ...
            #  prediction_scores.view(-1, self.config.vocab_size) : B,L,V-> B*L,V
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size),
                                      masked_lm_labels.view(-1))  # label  B, L -> B*L,1
    ...
    Examples::
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss_fct = CrossEntropyLoss()
                loss = gamma * loss_fct(logits.view(-1, self.num_labels),
                                      labels.view(-1)) + round((1 - gamma),1) * loss_root
