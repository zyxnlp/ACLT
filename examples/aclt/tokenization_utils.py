...
class PreTrainedTokenizer(object):
    ...
    def encode_plus(
        ...
        **kwargs
        ...
        def get_input_ids(text,label_roots,root_label_map,is_text_b=False):
            if isinstance(text, str):# todo tokenize

                words = text.split() # [a,b,c]
                roots = label_roots.split()
                if is_text_b:
                    roots=roots*len(words)

                tokens = []
                root_ids = []
                for word, root in zip(words, roots):
                    word_tokens = self.tokenize(word,**kwargs)

                    # bert-base-multilingual-cased sometimes output "nothing ([]) when calling tokenize with just a space.
                    if len(word_tokens) > 0:
                        tokens.extend(word_tokens)

                        root_ids.extend([root_label_map[root]] + [0] * (len(word_tokens) - 1))# binary cross entropy

                assert len(tokens)==len(root_ids)
                return self.convert_tokens_to_ids(tokens),root_ids
        ...
        first_ids,first_roots = get_input_ids(text,label_roots,root_label_map,False)
        second_ids,second_roots = get_input_ids(text_pair,'Aspect',root_label_map,True) if text_pair is not None else None
    ...
    def prepare_for_model(
        ...
        if max_length and total_len > max_length: #
            ids, pair_ids,first_roots,second_roots, overflowing_tokens = self.truncate_sequences(
                first_roots,
                second_roots,
                ids,
                pair_ids=pair_ids,
        ...
        if add_special_tokens:
            ...
            sequence_root = self.build_root_with_special_tokens(first_roots,second_roots)
            token_type_ids = self.create_token_type_ids_from_sequences(ids, pair_ids)# text_b+ sep 为1，cls+text_a+sep 为0
            assert len(sequence)==len(sequence_root)==len(token_type_ids)
        else:
        ...
        encoded_inputs["root_ids"] = sequence_root
        ...
        if max_length and len(encoded_inputs["input_ids"]) > max_length:
            ...
            encoded_inputs["root_ids"] = encoded_inputs["root_ids"][:max_length]
    ...
    def truncate_sequences(
        self,first_roots,second_roots, ids, pair_ids=None, num_tokens_to_remove=0, truncation_strategy="longest_first", stride=0
    ):
        ...
        if truncation_strategy == "longest_first":
            ...
            for _ in range(num_tokens_to_remove):
                ...
                if pair_ids is None or len(ids) > len(pair_ids):
                    ...
                    ids = ids[:-1]
                    first_roots=first_roots[:-1]

                else:
                    pair_ids = pair_ids[:-1]
                    second_roots=second_roots[:-1]
        ...
        return (ids, pair_ids,first_roots,second_roots, overflowing_tokens)
